...
const AGENTCHAT_URL = process.env.AGENTCHAT_URL || "wss://localhost:6667";
...
function loadAgentNames() {
	try {
		if (existsSync(AGENT_NAMES_FILE)) {
			...
			console.log(
				`Loaded ${Object.keys(agentNameOverrides).length} agent name overrides`,
			);
		}
	} catch (e) {
	...
	switch (msg.type) {
		...
		case "AGENTS": {
			...
			if (msg.channel && agentList.length > 0) {
				...
				broadcastToDashboards({
					type: "agents_update",
					data: [...state.agents.values()].map((a) => ({
						...a,
						channels: [...a.channels],
					})),
				});
		...
		case "AGENT_JOINED": {
			...
			broadcastToDashboards({
				...
				data: {
					...joiningAgent,
					channels: [...joiningAgent.channels],
					event: "joined",
				},
			});
			break;
...
function broadcastToDashboards(msg) {
	...
	if (msg.type === "message") {
		console.log(
			`Broadcast ${msg.type} to ${sent}/${dashboardClients.size} clients`,
		);
	}
}
...
function getStateSnapshot() {
	return {
		...
		messages: Object.fromEntries(
			[...state.channels.entries()].map(([name, ch]) => [
				name,
				ch.messages.toArray(),
			]),
		),
...
function handleDashboardMessage(client, msg) {
	switch (msg.type) {
		case "send_message": {
			if (client.mode === "lurk") {
				client.ws.send(
					JSON.stringify({
						type: "error",
						data: { code: "LURK_MODE", message: "Cannot send in lurk mode" },
					}),
				);
				return;
			}
			...
			client.ws.send(
				JSON.stringify({ type: "message_sent", data: { success: true } }),
			);
			break;
		}
		...
		case "set_mode":
			...
			client.ws.send(
				JSON.stringify({ type: "mode_changed", data: { mode: client.mode } }),
			);
			break;
		...
		case "join_channel":
			if (client.mode === "lurk") {
				client.ws.send(
					JSON.stringify({
						type: "error",
						data: { code: "LURK_MODE", message: "Cannot join in lurk mode" },
					}),
				);
				return;
			}
		...
		case "accept_proposal":
			if (client.mode === "lurk") {
				client.ws.send(
					JSON.stringify({
						type: "error",
						data: { code: "LURK_MODE", message: "Cannot accept in lurk mode" },
					}),
				);
				return;
			}
			...
			client.ws.send(
				JSON.stringify({
					...
					data: {
						...
						message: "Proposal actions require signing",
					},
				}),
			);
			break;
		...
		case "set_agent_name": {
			...
			if (agentId && name) {
				...
				try {
					writeFileSync(
						AGENT_NAMES_FILE,
						JSON.stringify(agentNameOverrides, null, 2),
					);
				} catch (e) {
				...
				broadcastToDashboards({
					...
					data: state.agents.has(agentId)
						? {
								...
								channels: [...state.agents.get(agentId).channels],
								event: "renamed",
							}
				...
				client.ws.send(
					JSON.stringify({
						type: "name_set",
						data: { agentId, name, success: true },
					}),
				);
...
app.use((req, res, next) => {
	res.set({
		...
		"Content-Security-Policy":
			"default-src 'self'; connect-src 'self' wss: ws:; script-src 'self'; style-src 'self' 'unsafe-inline'",
	});
	next();
});
...
wss.on("connection", (ws, req) => {
	...
	if (dashboardClients.size >= 100) {
		ws.send(
			JSON.stringify({
				type: "error",
				data: { code: "SERVER_FULL", message: "Too many clients" },
			}),
		);
		ws.close();
		return;
	...
	ws.on("message", (data) => {
		...
		try {
			...
			ws.send(
				JSON.stringify({
					type: "error",
					data: { code: "INVALID_MESSAGE", message: "Malformed message" },
				}),
			);
		}
	});
